    ...
    function logout(){
      ...
      useEffect(()=>{
        ...
        ws.onmessage = (event) => {
          ...
          } else if (JSON.parse(event.data).type === 'history') {
            const historyData = JSON.parse(event.data).data;
            const newMessages = historyData.map((msg: { name: string; message: string }) =>
              JSON.stringify({ name: msg.name, message: msg.message })
            );
      ...
      return (
        ...
        <div className="flex justify-between items-center mr-0 p-4 border-b border-gray-700">
         ...
         {/* Main Chat Area */}
      <div className="flex-1 flex flex-col max-h-[800px]">
        
        <ScrollArea className="flex-1 px-4"  style={{ maxHeight: '825px' }}>
        ...
        <div className="p-4 border-t fixed mt-[825px] w-screen bg-background/95 backdrop-blur  supports-[backdrop-filter]:bg-background/60">
          <div className="flex space-x-2 max-w-[1200px] mx-auto">
            <Input
              ref={inputRef}
              placeholder="Type your message..."
              className="flex-1 border-black -ml-90 border-2 h-10 w-"
